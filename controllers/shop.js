...
exports.getProduct = (req, res, next) => {
  ...
  Product.findAll({where:{id:prodId}}).then((products)=>{
    res.render("shop/product-detail", {
      product: products[0],
      pageTitle: products[0].title,path: "/products",
    });
  }).catch((err)=>{
    console.log(err);
  });
...
exports.getCart = (req, res, next) => {
  ...
  req.user.getCart().then((cart)=>{return cart.getProducts().then((products)=>
    {res.render("shop/cart", {path: "/cart", pageTitle: "Your Cart", products: products,})}).catch((err)=>{console.log(err);});}).catch((err)=>{console.log(err);});
...
exports.getOrders = (req, res, next) => {
  ...
  req.user.getOrders({include:["products"]}).then(orders=>{ //we have to include products to get them along with orders in the ejs
